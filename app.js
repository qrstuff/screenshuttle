...
const handler = async (event) => {
    ...
    if (!supportedFormats.includes(format)) {
        return {
            statusCode: 400,
            body: JSON.stringify({ error: 'Invalid file type. Only supported formats are ' + supportedFormats.join(',') }),
        };
    }

    if (format !== 'pdf' && (isNaN(width) || isNaN(height) || width <= 0 || height <= 0)) {
        return {
            statusCode: 400,
    ...
    try {
        const buffer = await capture(url, format, selector, exclude, width, height, fullpage);
        return {
            statusCode: 200,
            headers: {
